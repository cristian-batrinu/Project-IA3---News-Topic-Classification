...
def eval_model(model, dataloader, device):
    ...
    with torch.no_grad():
        for batch_data in tqdm(dataloader, desc="Eval"):
            if len(batch_data) == 3:
                emb_batch, label_batch, _ = batch_data
            else:
                emb_batch, label_batch = batch_data
...
if __name__ == '__main__':
    ...
    train_subset, _ = train_test_split(train_subset, test_size=0.2, stratify=train_subset['label'], random_state=42)
    ...
    train_dataset = EmbeddingTextDataset(train_emb, train_labels, train_texts)
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        for batch_data in tqdm(train_dl, desc=f"Epoch {epoch+1}"):
            ...
            num_adv = max(1, int(BATCH_SIZE * ADV_FRACTION))
            batch_size = emb_batch.size(0)
            
            adv_emb_list = []
            adv_label_list = []
            
            for i in range(min(num_adv, batch_size)):
                try:
                    text = texts_batch[i]
            ...
            if adv_emb_list:
                ...
                adv_label_batch = torch.tensor(adv_label_list, dtype=torch.long).to(device)
                combined_emb = torch.cat([adv_emb_batch, emb_batch[num_adv:]], dim=0)
                combined_labels = torch.cat([adv_label_batch, label_batch[num_adv:]], dim=0)
            else:
                combined_emb = emb_batch
                combined_labels = label_batch
            
            combined_labels = torch.clamp(combined_labels, 0, 3)
            combined_emb = combined_emb.unsqueeze(1)
            combined_size = combined_emb.size(0)
            attn_mask = torch.ones(combined_size, 1, dtype=torch.long, device=device)
            
            outputs = model(inputs_embeds=combined_emb, attention_mask=attn_mask, labels=combined_labels)
            loss = outputs.loss
